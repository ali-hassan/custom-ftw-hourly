...
import { LINE_ITEM_DAY, LINE_ITEM_NIGHT, LINE_ITEM_UNITS, DATE_TYPE_DATE, LINE_ITEM_ADDON_FEE, LINE_ITEM_ADDON_FEE_1, LINE_ITEM_ADDON_FEE_2 } from '../../util/types';
...
const estimatedTotalPrice = (unitPrice, unitCount, addonFee, addonFee_1, addonFee_2 ) => {
  const numericPrice = convertMoneyToNumber(unitPrice);
  const addonFeePrice = addonFee
    ? convertMoneyToNumber(addonFee)
    : null;
  const addonFeePrice_1 = addonFee_1
    ? convertMoneyToNumber(addonFee_1)
    : null;
  const addonFeePrice_2 = addonFee_2
    ? convertMoneyToNumber(addonFee_2)
    : null;

  const numericTotalPrice = new Decimal(numericPrice)
        .times(unitCount)
        .plus(addonFeePrice || 0)
        .plus(addonFeePrice_1 || 0)
        .plus(addonFeePrice_2 || 0)
        .toNumber()
...
const estimatedTransaction = (unitType, bookingStart, bookingEnd, unitPrice, quantity, addonFee, addonFee_1, addonFee_2 ) => {
  const now = new Date();
  ...
  const totalPrice = estimatedTotalPrice(unitPrice, unitCount, addonFee, addonFee_1, addonFee_2);
  ...
  const addonFeeLineItem = {
    code: LINE_ITEM_ADDON_FEE,
  ...
  const addonFeeLineItem_1 = {
    code: LINE_ITEM_ADDON_FEE_1,
    includeFor: ['customer', 'provider'],
    unitPrice: addonFee_1,
    quantity: new Decimal(1),
    lineTotal: addonFee_1,
    reversal: false,
  };

  const addonFeeLineItem_2 = {
    code: LINE_ITEM_ADDON_FEE_2,
    includeFor: ['customer', 'provider'],
    unitPrice: addonFee_2,
    quantity: new Decimal(1),
    lineTotal: addonFee_2,
    reversal: false,
  };

  const addonFeeLineItemMaybe = addonFee
    ? [addonFeeLineItem]
    : [];

  const addonFeeLineItemMaybe_1 = addonFee_1
    ? [addonFeeLineItem_1]
    : [];

  const addonFeeLineItemMaybe_2 = addonFee_2
    ? [addonFeeLineItem_2]
    : [];
...
const EstimatedBreakdownMaybe = props => {
  const { unitType, unitPrice, startDate, endDate, quantity, addonFee, addonFee_1, addonFee_2} = props.bookingData;
  ...
  const tx = estimatedTransaction(unitType, startDate, endDate, unitPrice, quantity, addonFee, addonFee_1, addonFee_2);
