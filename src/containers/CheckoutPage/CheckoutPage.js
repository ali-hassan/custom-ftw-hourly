...
import { propTypes, LINE_ITEM_NIGHT, LINE_ITEM_DAY, LINE_ITEM_ADDON_FEE, DATE_TYPE_DATE } from '../../util/types';
import {
  ensureListing,
  ensureCurrentUser,
  ensureUser,
...
export class CheckoutPageComponent extends Component {
  ...
  customPricingParams(params) {
    ...
    const addonFeeLineItem = addonFee
      ? {
      ...
      : null;
    const addonFeeLineItem_1 = addonFee_1
      ? {
          code: LINE_ITEM_ADDON_FEE,
      ...
      : null;
    const addonFeeLineItem_2 = addonFee_2
      ? {
          code: LINE_ITEM_ADDON_FEE,
    ...
    const addonFeeLineItemMaybe = addonFeeLineItem ? [addonFeeLineItem] : [];
    const addonFeeLineItemMaybe_1 = addonFeeLineItem_1 ? [addonFeeLineItem_1] : [];
    const addonFeeLineItemMaybe_2 = addonFeeLineItem_2 ? [addonFeeLineItem_2] : [];
  ...
  loadInitialData() {
    ...
    if (shouldFetchSpeculatedTransaction) {
      ...
      const addonFee = pageData.bookingData.addonFee;
      const addonFee_1 = pageData.bookingData.addonFee_1;
      const addonFee_2 = pageData.bookingData.addonFee_2;
  ...
  handlePaymentIntent(handlePaymentParams) {
    ...
    const addonFeeLineItem = speculatedTransaction.attributes.lineItems.find(
      item => item.code === LINE_ITEM_ADDON_FEE
    );
    const addonFeeLineItem_1 = speculatedTransaction.attributes.lineItems.find(
      item => item.code === LINE_ITEM_ADDON_FEE
    );
    const addonFeeLineItem_2 = speculatedTransaction.attributes.lineItems.find(
      item => item.code === LINE_ITEM_ADDON_FEE
    );
    const addonFee = addonFeeLineItem ? addonFeeLineItem.unitPrice : null;
    const addonFee_1 = addonFeeLineItem_1 ? addonFeeLineItem_1.unitPrice : null;
    const addonFee_2 = addonFeeLineItem_2 ? addonFeeLineItem_2.unitPrice : null;
