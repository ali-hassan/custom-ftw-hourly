...
export const validURLParamForExtendedData = (paramName, paramValueRaw, filters) => {
  ...
  if (paramName == 'pub_size' && !Array.isArray(filterConfig.options)) {
    var valid_sizes = Object.values(filterConfig.options).reduce((arr, new_arr)=> { return arr.concat(new_arr) }).map(el => { return {key: el, label: el} });
    filterConfig.options = valid_sizes;
  }
