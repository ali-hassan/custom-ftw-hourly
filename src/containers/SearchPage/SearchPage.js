...
export class SearchPageComponent extends Component {
  ...
  filters() {
    ...
    return {
      ...
      subCategoryFilter: {
        ...
        options: Object.values(subCategories).reduce((arr, new_arr)=> { return arr.concat(new_arr) }),
      },
      sizeFilter: {
        paramName: 'pub_size',
        options: sizes["excavators"],
      },
      amenitiesFilter: {
  ...
  render() {
    ...
    let urlQueryParams = pickSearchParamsOnly(searchInURL, filters);

    const selectedFilters = validFilterParams(urlQueryParams, filters);

    if (selectedFilters.pub_category && subCategories[selectedFilters.pub_category]) {
      filters.subCategoryFilter.options = subCategories[selectedFilters.pub_category]
    } else {
      delete filters.subCategoryFilter;
    }

    if ((selectedFilters.pub_category && selectedFilters.pub_sub_category) || (selectedFilters.pub_category && !filters.subCategoryFilter) ) {
      const size_key = selectedFilters.pub_category && selectedFilters.pub_sub_category ? `${selectedFilters.pub_category}-${selectedFilters.pub_sub_category}` : selectedFilters.pub_category;
      filters.sizeFilter.options = sizes[size_key].map(el => { return {key: el, label: el} })
    } else {
      delete filters.sizeFilter;
    }
    ...
    if (filters.subCategoryFilter) {
      primaryFilters.subCategoryFilter = filters.subCategoryFilter;
    }

    if (filters.sizeFilter) {
      primaryFilters.sizeFilter = filters.sizeFilter
    }

    // Update params after new filters added
    urlQueryParams = pickSearchParamsOnly(searchInURL, filters);
...
SearchPageComponent.defaultProps = {
  ...
  subCategories: config.custom.sub_categories,
